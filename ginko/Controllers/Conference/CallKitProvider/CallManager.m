...
- (CXProvider*)provider {
    if (!_provider) {
        CXProviderConfiguration *configuration = [[CXProviderConfiguration alloc] initWithLocalizedName:@"Ginko"];
        configuration.supportsVideo = YES;
        configuration.maximumCallsPerCallGroup = 1;
        configuration.supportedHandleTypes = [NSSet setWithObject:@(CXHandleTypePhoneNumber)];
        NSData *data = UIImagePNGRepresentation([UIImage imageNamed:@"callingIcon.png"]);
...
- (void)provider:(CXProvider *)provider performAnswerCallAction:(CXAnswerCallAction *)action {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(callDidAnswer)]) {
        ...
        [self.delegate callDidAnswerConnecting:action];
    }
}
