...
static CLLocationDistance distance = 1.0;
...
{
  if( self.leadObject ) {
      CLLocationCoordinate2D coord1 = self.leadObject.coordinate;
      CLLocationCoordinate2D coord2 = insertableObject.coordinate;
      CLLocationDistance dist = CLMetersBetweenCoordinates(coord1, coord2);
      //When distance is zero!
    if( CLMetersBetweenCoordinates(self.leadObject.coordinate, insertableObject.coordinate) >= [QNode getMinDistance] ) {
...
+ (void)setMinDistance:(CLLocationDistance)dist {
    distance = dist;
}

+ (CLLocationDistance)getMinDistance {
    return distance;
}
@end
