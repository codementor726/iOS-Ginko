...
@interface VideoCameraController () <FilterViewDelegate, MPMediaPickerControllerDelegate, GPUImageMovieDelegate, UIScrollViewDelegate,UIAlertViewDelegate>
{
    GPUImageMovie *movie;
...
- (void)loadView
{
    ...
    if (_isSetup) {
        [btnForApply setImage:[UIImage imageNamed:@"Checkmark_white.png"] forState:UIControlStateNormal];
    }else {
        [btnForApply setImage:[UIImage imageNamed:@"btn_apply"] forState:UIControlStateNormal];
    }
...
- (void)viewDidLoad
{
    ...
    [viewForVideo addGestureRecognizer:tapGesture];
...
{
    [self performSelector:@selector(play) withObject:nil afterDelay:1.0f];
}
...
- (void)startRecording
{
    ...
    VideoSegment *segment = [[VideoSegment alloc] init];
...
{
    VideoSegment *segment = [segments lastObject];
...
- (void)playSegment
{
    VideoSegment *segment = [segments objectAtIndex:current];
...
- (void)mergeVideos
{
    ...
    for (VideoSegment *segment in segments) {
        // Video;
        {
        ...
        if (!audio) {
            if (segment.audio) {
                AVAsset *audioAsset = [[AVURLAsset alloc] initWithURL:segment.audioURL options:nil];
...
//    exportSession.outputFileType = AVFileTypeQuickTimeMovie;
    if ([segments count] == 1 && !((VideoSegment *)[segments objectAtIndex:0]).audio) {
