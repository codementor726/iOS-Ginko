...
- (void)initializator
{
	...
	lstSelected = [[NSMutableArray alloc] init];
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        return MAX([UIBubbleTypingTableViewCell height], self.showAvatars ? 62 : 0);
    }
    ...
    return MAX(data.insets.top + data.view.frame.size.height + data.insets.bottom + 10, self.showAvatars ? 62 : 0);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if ([lstSelected containsObject:[NSString stringWithFormat:@"%@",data.msg_id]]) {
		cell.bSelected = YES;
	}else{
		cell.bSelected = NO;
	}
...
{
	if (status == 0) {
		[lstSelected removeObject:msgid];
	}else{
		[lstSelected addObject:msgid];
	}
	[bubbledelegate checkAction:msgid :status];
}

-(void)profileAction:(NSString *)userid
{
	[bubbledelegate profileAction:userid];
}
